...
export function AddTaskSheet({
  ...
  onClose: () => void;
  userId: string;
  onCreated: (task: PersonalTask) => void;
...
function AddTaskForm({
  ...
  const canSave = title.trim().length > 0 && !weeklyInvalid && !saving;
  ...
  async function handleSave() {
    ...
    if (error || !data) {
      setError(error?.message ?? "Could not save the task.");
      setSaving(false);
      return;
    }
    ...
    onCreated({
      ...
      repeat_dom: data.repeat_dom,
      is_done: data.is_done,
      last_done_on: data.last_done_on,
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
              ...
              {showNote ? (
                ...
                <button
                  ...
                  className="mt-1.5 inline-flex items-center gap-1 text-xs font-medium text-muted-foreground transition hover:text-foreground"
                >
              ...
              <div className="mt-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  <button
                    type="button"
                    onClick={() => { setCustomMode(true); setCategory(""); }}
                    className={cn(
              ...
              <div className="mt-5">
                    ...
                    {DAY_LABELS.map((d, i) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => toggleDay(i)}
                          className={cn(
                            "grid size-9 flex-1 place-items-center rounded-full border text-xs font-semibold transition",
                            active
                              ? "border-foreground bg-foreground text-background"
                              : "border-border text-muted-foreground hover:border-foreground/30 hover:text-foreground",
                          )}
                ...
                {repeat === "monthly" && (
                  <div className="mt-3 flex items-center gap-2 text-sm text-muted-foreground">
                    <span>On day</span>
                    <select
                      value={repeatDom ?? Number(today.split("-")[2])}
                      onChange={(e) => setRepeatDom(Number(e.target.value))}
                      className="h-9 rounded-[8px] border border-input bg-transparent px-2 text-sm outline-none transition focus:border-foreground/30"
                    >
              ...
              {/* when (one-time only) + time */}
              <div className="mt-5 space-y-2">
                {repeat === "none" && (
                  <div className="rounded-2xl border border-border">
                    <button
                      type="button"
                      onClick={() => { setShowCal((s) => !s); setShowTime(false); }}
                ...
                <div className="rounded-2xl border border-border">
                  <button
                    type="button"
                    onClick={() => { setShowTime((s) => !s); setShowCal(false); }}
                    className="flex w-full items-center justify-between px-3 py-2.5 text-sm"
                  >
              ...
              <div className="mt-5">
                <button
                  type="button"
                  onClick={() => setImportance((p) => (p === "high" ? "normal" : "high"))}
                  className={cn(
              ...
              {error && (
                <p className="mt-4 rounded-xl bg-red-50 px-3 py-2 text-sm text-red-600 dark:bg-red-500/10 dark:text-red-400">{error}</p>
              )}
            </div>
            ...
            <div className="border-t border-border px-5 py-4">
              ...
              {weeklyInvalid && (
                <p className="mt-2 text-center text-xs text-muted-foreground">Pick at least one day for a weekly task.</p>
              )}
            </div>
    </>
  );
